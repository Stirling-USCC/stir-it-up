...
`src/lib/game/createGame.js` owns all placeholder content and the tiny demonstration flow. Add or replace `Square` instances there, or define square subclasses and override async `onLand`, `onLeave`, and `onPass`. Add `Card` instances to a `Deck`; card hooks are `onDraw`, `onPlay`, and `onDiscard`. Add `Rule` objects with event handlers, or register new `Action` objects to change what the action panel offers. No UI change is needed to show new squares, players, stats, items, effects, decks, cards, log entries, or available actions. `StatsList.svelte` handles named values dynamically; `PlayerCard.svelte` shows inventories and effects in compact details.
